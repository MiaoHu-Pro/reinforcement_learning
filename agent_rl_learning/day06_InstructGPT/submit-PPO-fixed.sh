...
#SBATCH --job-name=gpt2-chinese-ppo
...
#SBATCH --output=result_out/gpt2-chinese-ppo-%j.out
...
RM_CHECKPOINT_PATH="${RM_OUTPUT_PATH}/reward_model.pt"
PPO_OUTPUT_PATH="${HOME}/scratch/llms_model/post_trained_models/gpt2-chinese-cluecorpussmall-ppo"
...
JOB_TEMP_DIR="${SLURM_TMPDIR:-/tmp}/gpt2-chinese-ppo-${SLURM_JOB_ID}"
...
echo "Input reward model: ${RM_CHECKPOINT_PATH}"
echo "PPO output: ${PPO_OUTPUT_PATH}"
...
if [[ ! -f "${TRAINING_DIR}/3-PPO-fixed.py" ]]; then
    echo "Training script not found: ${TRAINING_DIR}/3-PPO-fixed.py" >&2
    exit 1
fi

# PPO requires both earlier stages to have finished successfully.
...
if [[ ! -s "${RM_CHECKPOINT_PATH}" ]]; then
    echo "Reward-model checkpoint not found or empty: ${RM_CHECKPOINT_PATH}" >&2
    echo "Run submit-RM-fixed.sh successfully before this job." >&2
    exit 1
fi
...
srun "${PYTHON_EXECUTABLE}" 3-PPO-fixed.py

echo "Finished: $(date --iso-8601=seconds)"
echo "Saved PPO actor: ${PPO_OUTPUT_PATH}"
echo "Saved value head: ${PPO_OUTPUT_PATH}/value_head.pt"

#Run it on the server after SFT and RM finish:
#
#  cd ~/scratch/dips_project/reinforcement_learning/rl_learning_demo/day06_InstructGPT
#  mkdir -p result_out
#  sbatch submit-PPO-fixed.sh
#
#  If the earlier jobs are still running, submit PPO with dependencies:
#
#  sbatch --dependency=afterok:<SFT_JOB_ID>:<RM_JOB_ID> submit-PPO-fixed.sh
#
#  The script validates both checkpoints, activates rl_post_training_env, confirms GPU access, and saves the PPO model under:
#
