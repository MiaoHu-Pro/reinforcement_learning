...
for required_file in \
    config.py train_clip.py train_prior.py train_decoder.py dalle2_dataset.py; do
...
# Resolve the same experiment directory that every Python stage will use.
# The directory name is generated from the effective parameters unless the
# user supplies --run-name explicitly.
RUN_DIR=$("${PYTHON_EXECUTABLE}" config.py \
    --device cuda \
    --print-run-dir \
    "${USER_ARGS[@]}")
...
CHECKPOINTS=("${RUN_DIR}/prior.pt" "${RUN_DIR}/decoder.pt")
if [[ "${USING_PRETRAINED_CLIP}" == false ]]; then
    CHECKPOINTS=("${RUN_DIR}/clip.pt" "${CHECKPOINTS[@]}")
fi
for checkpoint in "${CHECKPOINTS[@]}"; do
    ...
    if [[ ! -s "${checkpoint}.complete" ]]; then
        echo "Training stage is not marked complete: ${checkpoint}.complete" >&2
        exit 1
    fi
    echo "Saved checkpoint: ${checkpoint}"
done
if [[ ! -s "${RUN_DIR}/effective_config.json" ]]; then
    echo "Training did not produce effective_config.json in ${RUN_DIR}" >&2
    exit 1
fi
echo "Saved configuration: ${RUN_DIR}/effective_config.json"
